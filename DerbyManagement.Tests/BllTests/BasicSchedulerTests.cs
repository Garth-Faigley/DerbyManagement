...
{
    [TestFixture]
    class BasicSchedulerTests
    {
        private IScheduler _basicRaceScheduler;

        [SetUp]
        public void SetUp()
        {
            SchedulerDetails[] schedulerDetails = RaceScheduler.GetAllSchedulers();
            _basicRaceScheduler = RaceScheduler.GetScheduler(schedulerDetails.Where(q => q.ClassName.Equals("BasicLaneRotation")).First());
        }
        ...
        [TestCase(2, 2, 0)]
        [TestCase(2, 2, 13)]
        public void BasicScheduler_InvalidInputData(int numberOfCars, int numberOfLanes, int runsPerLane)
        {
            try
            {
                var raceSchedule = _basicRaceScheduler.GetRaceSchedule(numberOfLanes, numberOfCars, runsPerLane, true);
